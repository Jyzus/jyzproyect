...
export default function IconTextFloat({name, icon, color, textColor, state}) {
	return (
		<div
			...
			} h-12 flex items-center justify-center relative bg-white hover:bg-principal group cursor-pointer transition-all duration-200 rounded-md`}
		>
			{/* Icon */}
			<div className={`${textColor} group-hover:text-white text-3xl`}>{icon}</div>
			{/* text */}
			<div
				className={`${
					state
			...
			>
				<p className={`${state ? 'text-dark group-hover:text-white' : 'text-white'} font-semibold`}>{name}</p>
			</div>
		</div>
	);
}
